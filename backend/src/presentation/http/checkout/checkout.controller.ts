...
export class CheckoutController {
  ...
  @ApiUnprocessableEntityResponse({
    description: inline(SWAGGER_RESPONSES.checkout.create.validation!),
  })
  @ApiServiceUnavailableResponse({
    description: inline(SWAGGER_RESPONSES.checkout.create.unavailable!),
  })
  async create(
  ...
  private ensureIdempotencyKey(key?: string): string {
    if (!key) {
      this.raise(
        ...
        asMessagePayload(CHECKOUT_MESSAGES.idempotencyKeyRequired),
      );
    }
  ...
  private dispatchCheckoutResponse(response: CheckoutResponseDto): CheckoutResponseDto {
    ...
    this.raise(
      ...
      asMessagePayload(CHECKOUT_MESSAGES.unexpectedError),
    );
  }

  private raise(
    status: HttpStatus,
    payload: CheckoutResponseDto | Record<string, unknown> | string,
  ): never {
